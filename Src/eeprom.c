...
static void init_header() {
  ...
  const Header new_header = {dummy_date, EEPROM_COMPLETIONS_ADDR,
                             EEPROM_COMPLETIONS_ADDR + sizeof(DayRecord),
                             false};
  eeprom_header = new_header;
...
void eeprom_init() {
  ...
  if (eeprom_header.date_bcd != curr_date) {
    ...
    eeprom_header.last_addr = eeprom_header.today_addr + sizeof(DayRecord);
    eeprom_header.date_bcd = curr_date;
    eeprom_write(EEPROM_HEADER_ADDR, sizeof(Header), (char *)&eeprom_header);
  }
}
Header get_header() { return eeprom_header; }
